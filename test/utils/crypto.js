...
import { createECKey, createHS256JWK, parseAlgorithmIdentifier } from '../../lib/jwa.js';
import { generateECDSA, importJWK, jwkFromPKCS8, pkcs8FromJWK } from '../../utils/crypto.js';
import { derFromPEM, derFromPKCS8, formatPem, pemFromPKCS8 } from '../../utils/pkcs.js';
...
test('pkcs8FromJWK() - rsa', async (t) => {
  ...
  t.is(formatPem(pem), result);
});

test('jwkFromPKCS8() - auto select algorithm (rsa)', async (t) => {
  const pem = `-----BEGIN PRIVATE KEY-----
    MIIEvQIBADANBgkqhkiG9w0BAQEFAASCBKcwggSjAgEAAoIBAQDD0tPV/du2vftjvXj1t/gXTK39
    sNBVrOAEb/jKzXae+Xa0H+3LhZaQIQNMfACiBSgIfZUvEGb+7TqXWQpoLoFR/R7MvGWcSk98JyrV
    tveD8ZmZYyItSY7m2hcasqAFiKyOouV5vzyRe87/lEyzzBpF3bQQ4IDaQu+K9Hj5fKuU6rrOeOhs
  ...
  const jwk = await jwkFromPKCS8(der);
  t.deepEqual(
    jwk,
    {
      kty: 'RSA',
...
test('generateEC Key', async (t) => {
  const jwk = await generateECDSA();
  console.log(jwk);
  const der = pkcs8FromJWK(jwk, parseAlgorithmIdentifier(jwk));
  console.log(der);
});

// test('jwkFromPKCS1()', async (t) => {
...
//   -----END RSA PRIVATE KEY-----`;
//   const jwk = await jwkFromPKCS1(pem, { name: 'RSASSA-PKCS1-v1_5', hash: 'SHA-256' });
//   t.is(jwk.kty, 'RSA');
//   t.is(jwk.alg, 'RS256');
