...
import { extractPublicJWK, isPrivate, isSymmetric, parseAlgorithmIdentifier } from './jwa.js';
...
export default class KeyStore {
  ...
  async importJWK(jwk, algorithmIdentifier) {
    ...
    let privateKey;
    if (isSymmetric(jwk)) {
      const key = await importJWK({ ...jwk, key_ops: ['sign', 'verify'] }, parsedAlgorithmIdentifier);
      publicKey = key;
      privateKey = key;
    } else if (isPrivate(jwk)) {
      privateKey = await importJWK({ ...jwk, key_ops: ['sign'] }, parsedAlgorithmIdentifier);
      publicKey = await importJWK({ ...extractPublicJWK(jwk), key_ops: ['verify'] }, parsedAlgorithmIdentifier);
    } else {
      publicKey = await importJWK({ ...jwk, key_ops: ['verify'] }, parsedAlgorithmIdentifier);
    }
  ...
  async encrypt(jwk, data) {
    const cacheItem = await this.retrieveKey(jwk, true);
    return await encrypt(cacheItem.algorithmIdentifier, cacheItem.privateKey, data);
  }
}
